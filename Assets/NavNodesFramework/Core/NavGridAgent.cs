...
namespace NavNodesFramework.Core
{
	...
	public class NavGridAgent : MonoBehaviour
	{
		...
		[Tooltip("How many nodes per second to walk")] 
		public float walkSpeed = .25f;
		[Tooltip("How fast should the agent lerp to its current node.")] 
		public float lerpAmount = .25f;
		[Tooltip("Should the Agent turn to face the direction last traveled in.")]
		public bool turnInDirectionOfTravel;
		[SerializeField, Tooltip("Should the Agent turn to face the direction last traveled in.")] 
		private float turnLerpSpeed = 0.2f;
	...
	#region MoveToTargetOverrides
		...
		/// step the agent all the way to the given target.
		/// </summary>
		public void MoveToTarget(NavGridNode _target, bool _accountForBlocks = false)
		{
			StopAllCoroutines();
			StartCoroutine(WalkTowardsTarget(_target, _accountForBlocks));
		}

		/// <summary>
		/// step the agent along to the given target for _maxTilesToMove blocks
		/// </summary>
		public void MoveToTarget(NavGridNode _target, int _maxTilesToMove, bool _accountForBlocks = false)
		{
			StopAllCoroutines();
	        StartCoroutine(WalkTowardsTarget(_target, _maxTilesToMove, _accountForBlocks));
		}

		/// <summary>
		/// step the agent all the way to node closest to the _target GameObject.
		/// </summary>
		public void MoveToTarget(GameObject _target, bool _accountForBlocks = false)
		{
			if(navGrid.NodesByGameObject.ContainsKey(_target))
			{
				NavGridNode nodeTarget = navGrid.NodesByGameObject[_target];
				StopAllCoroutines();
				StartCoroutine(WalkTowardsTarget(nodeTarget, _accountForBlocks));
			}
		}
		...
		/// </summary>
		public void MoveToTarget(GameObject _target, int _maxTilesToMove, bool _accountForBlocks = false)
		{
			if(navGrid.NodesByGameObject.ContainsKey(_target))
			{
				NavGridNode nodeTarget = navGrid.NodesByGameObject[_target];
				StopAllCoroutines();
				StartCoroutine(WalkTowardsTarget(nodeTarget, _maxTilesToMove, _accountForBlocks));
			}
		}
		...
		/// </summary>
		public void MoveToTarget(Vector3 _target, bool _accountForBlocks = false)
		{
		...
		public void MoveToTarget(Vector3 _target, int _maxTilesToMove, bool _accountForBlocks = false)
		{
			NavGridNode nodeTarget = navGrid.ClosestNavGridNodeToPosition(_target);
			StopAllCoroutines();
			StartCoroutine(WalkTowardsTarget(nodeTarget, _maxTilesToMove, _accountForBlocks));
	...
	#region NumberOfNodesToTargetOverrides
		...
		/// </summary>
		public int NumberOfNodesToTarget(NavGridNode _target, bool _accountForBlocks = false)
		{
			return CalculatePathToTarget(currentNode, _target, _accountForBlocks).Count;
		}
		
		/// <summary>
		/// returns the number of nodes to the node closest to the _target gameobject
		/// </summary>
		public int NumberOfNodesToTarget(GameObject _target, bool _accountForBlocks = false)
		{
			if(navGrid.NodesByGameObject.ContainsKey(_target))
			{
				NavGridNode nodeTarget = navGrid.NodesByGameObject[_target];
				return CalculatePathToTarget(currentNode, nodeTarget, _accountForBlocks).Count;
			}
			return 0;
		}
		...
		/// </summary>
		public int NumberOfNodesToTarget(Vector3 _target, bool _accountForBlocks = false)
		{
			NavGridNode nodeTarget = navGrid.ClosestNavGridNodeToPosition(_target);
			return CalculatePathToTarget(currentNode, nodeTarget, _accountForBlocks).Count;
		}
